...
**当前实现**：基于知识库（ES + Qdrant）实现图谱增强 RAG。

**Neo4j 集成规划（2026-04-28 新增）**：向量检索 + Neo4j 图数据库结合使用。
...
| `code/backend/app/services/neo4j_service.py` | Neo4j 服务（待开发） |
| `code/backend/app/services/graph_extractor.py` | 实体关系抽取（待开发） |
...
## 八、Neo4j 集成开发指南（2026-04-28 新增）

### 8.1 Neo4j Python 客户端
...
class Neo4jService:
    ...
    def create_relation(self, from_name, from_label, to_name, to_label, rel_type, rel_props=None):
        query = f"""
        ...
        """
        with self.driver.session() as session:
            result = session.run(query, from_name=from_name, to_name=to_name, rel_props=rel_props or {})
            return result.single()
```
...
```python
EXTRACT_PROMPT = """从以下文本中提取关键实体及其类型，以及实体之间的关系。

实体类型包括：
- Law: 法规名称（如《噪声污染防治法》）
- Article: 条款编号（如第2条、第3条第1款）
- Standard: 技术标准编号（如GB 3096-2008）
- Penalty: 处罚内容（如罚款5000-50000元）
- Case: 案例名称（如XX项目噪声超标案）
- Subject: 主体（如建设单位、施工单位）
- Behavior: 行为（如噪声超标排放）
...
MATCH (law:Law {name: $law_name})
      ...
      -[:DEFINES]->(behavior:Behavior)
      -[:TRIGGERS]->(penalty:Penalty)
RETURN law, article, behavior, penalty

# 查询两实体间的最短路径
MATCH path = shortestPath(
  (a {name: $from_name})-[*]-(b {name: $to_name})
)
RETURN path

# 子图查询：获取实体周围2跳内的所有节点和关系
MATCH (e {name: $entity_name})
MATCH path = (e)-[*..2]-(connected)
RETURN path
```
